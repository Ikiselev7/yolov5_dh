...
def run(
    ...
    if not training:
        if pt and not single_cls:  # check --weights are trained on --data
            ncm = model.model.nc[head] if head is not None else model.model.nc
    ...
    names = names[head] if head is not None else names
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            out, train_out = model(im, head=head) if compute_loss else (model(im, head=head, augment=augment), None)

        # Loss
        if compute_loss:
            loss += compute_loss(*train_out, targets, head=head)[1]  # box, obj, cls
...
def parse_opt():
    ...
    parser.add_argument('--head', type=int, default=None, help='head to infer')
