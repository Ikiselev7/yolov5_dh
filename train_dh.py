...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loaders = [create_dataloader(val_path,
                                       imgsz,
                                       ...
                                       workers=workers * 2,
                                       pad=0.5,
                                       prefix=colorstr('val: '))[0] for val_path in val_paths]
    ...
    # Start training
    t0 = time.time()
    nb = len(tr_loader)  # number of batches
    nw = max(round(hyp['warmup_epochs'] * nb), 100)  # number of warmup iterations, max(3 epochs, 100 iterations)
    # nw = min(nw, (epochs - start_epoch) / 2 * nb)  # limit warmup to < 1/2 of training
    last_opt_step = -1
    maps = [np.zeros(ncl) for ncl in nc]  # mAP per class
    results = [(0, 0, 0, 0, 0, 0, 0) for _ in range(len(nc))] # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps = [], []
                for i, (data_dict, val_loader) in enumerate(zip(data_dicts, val_loaders)):
                    save_val = save_dir / f"val_{i}"
                    save_val.mkdir(exist_ok=True, parents=True)
                    result, map, _ = validate.run(data_dict,
            ...
            # Update best mAP
            fi = sum(fitness(np.array(res).reshape(1, -1)) for res in results) / len(results)  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
